...
async def _demo_orchestrator(tmp_path: Path):
    ...
    # Keep real thresholds: 30 and 10 (not -10000)
    assert settings.transfer.min_net_profit_bps == D("30")
...
async def test_demo_controlled_profitable_plan_found(tmp_path: Path):
    ...
    try:
        ...
        assert len(plans) == 1, f"expected 1 profitable plan above 30 bps, got {plans}"
        plan = plans[0]
        assert plan.net_profit_bps >= D("30"), f"net {plan.net_profit_bps} <30"
